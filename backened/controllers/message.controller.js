...
export const saveMessage = async (req, res) => {
  try {
    const { carId, receiver, text, conversationId, isSystemMessage } = req.body;
...
// Get all conversations for a specific car
export const getCarConversations = async (req, res) => {
  try {
    const { carId } = req.params;
    const currentUserId = req.user.userId;
    ...
    // Find all unique conversationIds for this car
    const conversations = await Message.aggregate([
      {
        $match: {
      ...
      },
      {
        $project: {
          _id: 0,
          conversationId: 1,
          // Get the other user ID from the conversation ID
          otherUserId: {
            $cond: {
              if: { $eq: [{ $indexOfArray: [{ $split: ["$conversationId", "-"] }, currentUserId] }, 1] },
              then: { $arrayElemAt: [{ $split: ["$conversationId", "-"] }, 2] },
              else: { $arrayElemAt: [{ $split: ["$conversationId", "-"] }, 1] }
            }
          },
    ...
    console.error("Error getting car conversations:", error.message);
    res.status(500).json({ error: 'Failed to get car conversations.', details: error.message });
  }
};
