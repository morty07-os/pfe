...
// Car routes
router.post("/add", ProtectedRoute(), createCar); // Add a new car
router.get("/list", getCars); // Get a list of cars
router.put("/update/:id", ProtectedRoute(), updateCar); // Update car details
...
router.post('/addcars', ProtectedRoute(), async (req, res) => {
    try {
        ...
        const { carName, brand, wilaya, description, energy, seats, doors, transmission, mileage, engine, availabilityStart, availabilityEnd, price, carType, images, location } = req.body;
        ...
        const car = new Car({
            ...
            price,
            carType,
            images: images, // Use the Cloudinary URLs directly
            location: location, // Use the location data directly
